...
void LineDetection3D::run( std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> &planes_point_cloud, int k, 
                           ...
                           std::vector<double> &ts  )
{
	...
	planeBased3DLineDetection( planes_point_cloud, planes );
	timer.Stop();
	totalTime += timer.GetElapsedSeconds();
	timer.PrintElapsedTimeMsg(msg);
...
void LineDetection3D::planeBased3DLineDetection( std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> &planes_point_cloud, 
                                std::vector<PLANE> &planes)
{
	int numPatches = planes_point_cloud.size();

    std::vector<std::vector<int> > regions;

    // PointCloud<double> pointData;
    int all_pts_num = 0;
    for (auto& pc : planes_point_cloud) {
        all_pts_num += pc->size();
    }
    pointData.pts.reserve(all_pts_num);
    this->pointNum = all_pts_num;
...
// #pragma omp parallel for
	...
	{
		int pointNumCur = planes_point_cloud[i]->size();
        regions.push_back({});
        regions.back().reserve(pointNumCur);
		std::vector<std::vector<double> > pointDataCur(pointNumCur);
		for ( int j=0; j<pointNumCur; ++j )
		{
			pointDataCur[j].resize(3);
			pointDataCur[j][0] = planes_point_cloud[i]->points[j].x;
			pointDataCur[j][1] = planes_point_cloud[i]->points[j].y;
			pointDataCur[j][2] = planes_point_cloud[i]->points[j].z;
            pointData.pts.emplace_back(planes_point_cloud[i]->points[j].x, 
                                       planes_point_cloud[i]->points[j].y, 
                                       planes_point_cloud[i]->points[j].z);
            regions.back().push_back(count_pt);
            ++count_pt;
		}
	...
	pcaer.Ori_PCA( pointData, k, pcaInfos, scale, this->magnitd );

	double thAngle = 10.0/180.0*CV_PI;
	double thLineLength = scale;
...
void LineDetection3D::lineFromMask( cv::Mat &mask, int thLineLengthPixel, std::vector<std::vector<std::vector<cv::Point2d> > > &lines )
{
	...
	// cv::namedWindow("mask2", WINDOW_FREERATIO);
	// string mask_str2 = "mask2";
	// cv::imshow(mask_str2, mask2);
	// cv::waitKey(-1);
	...
	// cv::drawContours(mask2, contours, -1, cv::Scalar::all(250));
	// string mask_str3= "mask3";
	// cv::imshow(mask_str3, mask2);
	// cv::waitKey(-1);
